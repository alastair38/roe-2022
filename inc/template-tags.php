...
function blockhaus_display_social_profiles() {
	...
	$blueSky = get_field('bluesky_url', 'options');

	if($blueSky) {
		$blueSky_profile = '<li class="wp-social-link wp-block-social-link"><a href="' . $blueSky . '" class="wp-block-social-link-anchor"><svg xmlns="http://www.w3.org/2000/svg" width="256" height="226" viewBox="0 0 256 226">
...
</svg><span class="wp-block-social-link-label screen-reader-text">Jenny Roe on Blue Sky</span></a></li>';
	}
	...
	if($facebook || $instagram || $linkedin || $tiktok || $blueSky || $youtube):
		...
		$profiles .= $facebook_profile . $instagram_profile . $linkedin_profile . $tiktok_profile . $blueSky_profile . $youtube_profile;
		$profiles .= '</ul>';
	endif;
